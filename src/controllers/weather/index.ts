...
export default async function (discordClient: Client) {
  ...
  if (report.event) {
    const warnEmbed = new SunnyEmbed()
      ...
      .setAuthor("Ace Mansion Weather")
      .setTitle(
        `${report.event[0].toUpperCase() + report.event.slice(1)} Warning`
      )
      .setDescription(
        `Hi folks, we're just getting news of a **${report.event}** today!`
      )
      .setColor("#ff0505")
      .addField(
        "\u200b",
